<?php 
    ...
    if (mysqli_num_rows($result) > 0){
        ...
        while ($row = mysqli_fetch_assoc($result)){
            ...
            $hourly_rate = 100; // example ₹100 per hour

// convert work hours to decimal
            $work_hours = strtotime($row["work_hour"]) - strtotime("00:00:00");
            $work_hours = $work_hours / 3600;

// convert overtime
            $overtime_hours = strtotime($row["overtime"]) - strtotime("00:00:00");
            $overtime_hours = $overtime_hours / 3600;

            $salary = ($work_hours * $hourly_rate) + ($overtime_hours * $hourly_rate * 1.5);
            if($row["am_in"]==null){
                echo "<td>-</td>";
                }
